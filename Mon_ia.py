...
# 2. LE CSS "FORCE BRUTE" (Injection des couleurs et masquage)
# On force les variables de couleurs et on cache les éléments indésirables
force_css = """
    <style>
    /* ---------------------------------------------------- */
    /* 1. FORCER LE BLEU MARINE (Écraser le rouge par défaut) */
    /* ---------------------------------------------------- */

    /* La bordure des champs de texte quand on clique */
    ...
    /* Le bouton Envoyer (Avion) et les boutons primaires */
    button[kind="primary"], button[kind="primary"]:focus {
    ...
    /* L'icône SVG dans le bouton d'envoi */
    button[data-testid="stChatInputSubmitButton"] svg {
        fill: #0E2A47 !important;
    }

    /* ---------------------------------------------------- */
    /* 2. MASQUER LES LOGOS ET MENUS (Nettoyage) */
    /* ---------------------------------------------------- */
    ...
    /* Cacher la barre du haut et les options */
    [data-testid="stHeader"] {visibility: hidden; display: none;}
    [data-testid="stToolbar"] {visibility: hidden; display: none;}

    /* Cacher le bouton 'Hosted with Streamlit' spécifique */
    .viewerBadge_container__1QSob {display: none !important;}
    a[href*="streamlit.io/cloud"] {display: none !important;}

    /* Remonter le contenu pour cacher le vide laissé en haut */
    .stApp {
        margin-top: -80px;
...
if prompt := st.chat_input("Tapez vos questions..."):
    ...
    with st.chat_message("assistant", avatar="💼"):
        with st.spinner(""): # Le cercle va tourner ici
            reponse = client.chat.completions.create(
                model="gpt-4o",
                messages=st.session_state.messages
            )
            msg_ia = reponse.choices[0].message.content
        st.write(msg_ia) # Le texte s'affiche ici
